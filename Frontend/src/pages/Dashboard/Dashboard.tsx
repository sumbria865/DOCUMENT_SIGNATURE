...
export const Dashboard = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const pending = response.documents.filter(d => d.status === 'PENDING' || d.status === 'PARTIALLY_SIGNED').length;
      const signed = response.documents.filter(d => d.status === 'SIGNED').length;
      const rejected = response.documents.filter(d => d.status === 'REJECTED').length;
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard icon={FileText} label="Total Documents" value={stats.total} color="bg-blue-600" />
          ...
          <StatCard icon={AlertCircle} label="Rejected" value={stats.rejected} color="bg-red-500" />
        </div>
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-8 mb-8 text-white flex flex-col md:flex-row justify-between items-center">
          <div>
            ...
            <p className="text-blue-100">Upload a PDF and start the signing process</p>
          </div>
          ...
          <div className="flex gap-3 mt-4 md:mt-0">
            ...
            {/* ✅ TEST SIGN PAGE BUTTON */}
            <Button
              className="bg-black text-white hover:bg-gray-800"
              onClick={() => navigate("/sign/testtoken123")}
            >
              Open Sign Page (Test)
            </Button>
          </div>
                ...
                {documents.slice(0, 5).map((doc) => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h3 className="font-medium">Document #{doc.id.slice(0, 8)}</h3>
